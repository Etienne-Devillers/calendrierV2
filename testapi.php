<?php 
$curl = curl_init('https://data.education.gouv.fr/api/records/1.0/search/?dataset=fr-en-calendrier-scolaire&q=&facet=description&facet=population&facet=start_date&facet=end_date&facet=zones&facet=annee_scolaire&refine.location=Amiens&refine.zones=Zone+B&refine.annee_scolaire=2021-2022');
...
function createObjectHolidays($holidaysNameGroup, $holidaysArray, $value) {
...
foreach ($data['records'] as $key => $value) {
    ...
    if ($value['fields']['description'] == 'Vacances de Printemps' && $duplicateArray['spring'] != 1) {
        $tempObject = createObjectHolidays($tempObject, $holidaysArray, $value);
        array_push($holidaysArray, $tempObject);
